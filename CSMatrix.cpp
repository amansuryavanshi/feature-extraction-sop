...
void CSMatrix::countOccurrences(CSCol *csCol, Occurrence *occurrence, int minSetting_i, float magnitude, float rSquared,float adjustedRSquared) {
         if (occurrence->factorList_n == csCol->factors) {
		occurrence->rSquaredContribution += rSquared;
		occurrence->adjustedRSquaredContribution += adjustedRSquared;  // Update adjusted R-squared ADDED
	}
	float oldAdjustedRSquared = occurrence->adjustedRSquaredContribution;
    occurrence->adjustedRSquaredContribution += adjustedRSquared;
	...
	for (int setting_i = minSetting_i; setting_i < csCol->factors; setting_i++) {
		...
		countOccurrences(csCol, &occurrence->list[csCol->setting[setting_i].factor_i], setting_i + 1, magnitude, rSquared,adjustedRSquared);
	}
}
