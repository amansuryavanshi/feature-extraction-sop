...
Model::Model(VectorXf *response, int maxTerms, CSMatrix *csMatrix) {
	...
	hTermIndex->adjustedRSquared=this->adjustedRSquared; //ADDED
...
Model::Model(Model *model) {
	...
	this->adjustedRSquared=model->adjustedRSquared; //ADDED
	...
	for (TermIndex *pTermIndex = model->hTermIndex; pTermIndex != NULL;
		...
		(*destTermIndex)->termIndex = pTermIndex->termIndex;
				...
				(*destTermIndex)->adjustedRSquared=pTermIndex->adjustedRSquared;//ADDED
...
void Model::printModelFactors() {
	...
	cout << setw(15) << right << "R^2 Contr." << " | " << setw(15) << right << "Coefficient" << " | " << setw(15) << right << "Adj. R^2" << " | "<< "Term" << endl; //added
	...
	cout << setw(15) << setfill('-') << "" << " | "
         << setw(15) << setfill('-') << "" << " | "
         << setw(15) << setfill('-') << "" << " | "  // Add this line and changed and marked above
         << setfill(' ') << endl;
	...
	// }
	// COMMENTED ABOVE AND CHANGED
	 for (TermIndex *pTermIndex = hTermIndex; pTermIndex != NULL; pTermIndex = pTermIndex->next) {
        int termIndex = pTermIndex->termIndex;
        cout << setw(15) << right << pTermIndex->rSquared << " | "
             << setw(15) << right << coefVec[termIndex] << " | "
             << setw(15) << right << pTermIndex->adjustedRSquared << " | "  // Print adjusted R-squared for the term
             << csMatrix->getColName(csMatrix->getCol(termIndex)) << endl;
    }
	
	// calculate adjusted r-squared (terms - 2 to not include the intercept)
	//float adjustedRSquared = 1 - (1 - rSquared) * (tests - 1) / (tests - terms); // CONFLICT tests-terms-1?
...
void Model::leastSquares() {
	...
	*/
	...
	if(tests-terms!=0){
		adjustedRSquared= 1 - ((1-rSquared)*((float)tests-1)/((float)tests-(float)terms));  // ADDED ask ma'am about -1 or not
		//cout<<"this 2 "<<tests<<" "<<terms<<" "<<adjustedRSquared<<"\n";
	}else{
	    adjustedRSquared=0;
		//cout<<"this "<<tests<<" "<<terms<<"\n";
}
}
...
float Model::getAdjustedRSquared() {  //ADDED
...
bool Model::addTerm(int col_i) {
	...
	for (; ; pTermIndex = &(*pTermIndex)->next) {
		...
		if (*pTermIndex == NULL || (*pTermIndex)->termIndex > col_i) {
			...
			// calculate r-squared contribution for this term
			float oldRSquared = this->rSquared;
			float oldAdjustedRSquared = this->adjustedRSquared; //ADDED
			// cout<<oldAdjustedRSquared<<"\n";
			leastSquares();
			termIndex->rSquared = this->occurrences * (this->rSquared - oldRSquared);
			// int n = response->getLength();
            // int p = terms;  // Ensure this represents the total number of terms after adding the new one
            // adjustedRSquared = 1 - ((1 - rSquared) * (n - 1) / (n - p - 1));
			termIndex->adjustedRSquared = this->occurrences * (this->adjustedRSquared - oldAdjustedRSquared); //ADDED

			// this->occurrences * (this->adjustedRSquared - oldAdjustedRSquared)
			return true;
		} else if ((*pTermIndex)->termIndex == col_i) {
			// already exists so cannot insert
			return false;
	...
	for (TermIndex *pTermIndex = hTermIndex; pTermIndex != NULL; pTermIndex = pTermIndex->next) {
		int termIndex = pTermIndex->termIndex;
                float rSquared = pTermIndex->rSquared;
				 float adjustedRSquared = pTermIndex->adjustedRSquared;  // Use the adjusted R-squared contribution ADDED
		float magnitude = coefVec[term_i++];
		csMatrix->countOccurrences(csMatrix->getCol(termIndex), occurrence, 0, magnitude, rSquared,adjustedRSquared);
	}
}
...
bool Model::isDuplicate(Model *model, bool merge) {
	...
	if (merge) {
		...
		for (p1TermIndex = hTermIndex; p1TermIndex != NULL; p1TermIndex = p1TermIndex->next) {
			...
			p1TermIndex->adjustedRSquared+=p2TermIndex->adjustedRSquared;
