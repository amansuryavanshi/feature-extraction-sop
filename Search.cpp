...
void createModels(LocatingArray *locatingArray, VectorXf *response, CSMatrix *csMatrix,
	...
	while (topModels[0] != NULL && topModels[0]->getTerms() < maxTerms) {
			...
			//	cout << endl;
				...
				if (!isDuplicate) {
					...
					if (nextTopModels[models_n - 1] == NULL ||
						nextTopModels[models_n - 1]->getAdjustedRSquared() < newModel->getAdjustedRSquared()) { // CHANGED
					...
					for (int model_i = models_n - 2; model_i >= 0; model_i--) {
						if (nextTopModels[model_i] == NULL ||
							nextTopModels[model_i]->getAdjustedRSquared() < nextTopModels[model_i + 1]->getAdjustedRSquared()) { // CHANGED
		...
		if (topModels[0] != NULL) {
			...
			cout << "Top Model (" << topModels[0]->getAdjustedRSquared() << "):" << endl;
	...
	for (int model_i = 0; model_i < models_n; model_i++) {
		if (topModels[model_i] != NULL) {
			...
			cout << "Model " << (model_i + 1) << " (" << topModels[model_i]->getAdjustedRSquared() << "):" << endl; //ADDED
